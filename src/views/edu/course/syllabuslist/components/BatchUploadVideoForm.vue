<template>
    ...
    >
        ...
        <el-form :model="modelRef" :rules="rulesRef" ref="formRef" label-width="80px">
          ...
          <!--进度条-->
          <el-progress style="margin-top: 10px;margin-bottom: 10px;" v-show="showProgress" :text-inside="true" :stroke-width="24" :percentage="uploadPercent" status="success"></el-progress>
...
interface UpdateVideoFormSetupData {
    ...
    onFileChange: ( file: any, fileList: any) => void;
}
...
export default defineComponent({
    ...
    props: {
        ...
        getPercent: {
          type: Function as PropType<(values: TableListItem) => number>,
          required: true
        }
    },
    components: {
    ...
    setup(props): UpdateVideoFormSetupData {
     ...
     const getUploadPercent= async ()=>{
        const response: number = await props.getPercent(modelRef)
     ...
     const FileOnSuccess=(response: any, file: any, fileList: any)=>{
       ...
       const num: any= ((++sucFileNum.value)/(fileNum.value)).toFixed(2)
       uploadPercent.value=num*100
        // 进度条隐藏
        //showProgress.value = false
        // 重置uploadPercent上传进度条
        //this.resetPercent()

       if(sucFileNum.value===fileNum.value){
       ...
       //clearInterval(intervalId);
        ...
        return {
            ...
            resetFields,
            onFinish,
            uploadApi: process.env.VUE_APP_APIHOST+'edu/vod/batchUploadApi',
            handleProgress,
            onFileChange,
