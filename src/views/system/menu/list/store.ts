...
import { TableDataType, TableListItem, TableListQueryParams,MenuTableDataType} from './data.d';
import {
  queryList,
  removeData,
  createData,
...
export interface ModuleType extends StoreModuleType<StateType> {
    ...
    actions: {
        ...
        queryMenuTableData: Action<StateType, StateType>;
    };
}
...
const StoreModel: ModuleType = {
    ...
    mutations: {
        setTableData(state, payload) {
            // console.info("菜单数据setTableData：",payload)
        ...
        setMenuData(state, payload) {
            // console.info("菜单数据setMenuData：",payload)
    ...
    actions: {
        ...
        async createTableData({ commit }, payload: Pick<TableListItem, "name" | "svgIcon" | "component" | "path" | "isMenu"> ) {
            try {
        ...
        async queryMenuTableData({ commit }, payload: any ) {
            try {
                const response: ResponseData = await queryList(payload);
                const { data } = response;
                commit('setMenuData',{
